...
const getList = async ({
  ...
  const ret = await api.get(url, {
    ...
    source: 'WEB',
    client: 'WEB',
    filter: '(HOLDER_NEW="10000303290")(END_DATE>=\'2015-03-31\')',
    reportName: 'RPT_F10_EH_FREEHOLDERS'
  })
  logger.info('morganStanley', startDate, endDate, page, JSON.stringify(ret))
  if (ret.code === 0) {
    callback(ret)
  }
...
const start = async () => {
  ...
  const filePath = path.resolve(__dirname, '../json/morganStanley/data.json')
  logger.info('morganStanley', list.length)
  fs.writeFileSync(
    filePath,
